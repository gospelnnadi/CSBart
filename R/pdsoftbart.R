#' Partial dependence plots for CSBart
#' 
#' Modified version of the \code{pdbart} function from the \code{BayesTree}
#' package; largely supplanted by the \code{csbart_regression} and
#' \code{partial_dependence_regression} functions. Runs \code{csbart} at test
...
#' @param ... Additional arguments passed to csbart or plot.
#' 
...
#' }
pdcsbart <- function(X, Y, xind = NULL, levs = NULL,
...
)
{
  ...
  pdbrt = csbart(x.train,y.train,x.test,...)
  if (ncol(x.train)==2) {
    fdr = pdbrt$yhat.test
  } else {
    fdr = NULL 
