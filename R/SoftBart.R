...
#' @param weights Only used by the function \code{csbart}, this is a vector of weights to be used in heteroskedastic regression models, with the variance of an observation given by \code{sigma_sq / weight}.
...
#' 
#' ## Fit the model
#' fit <- csbart(X = sim_data$X, Y = sim_data$Y, X_test = sim_data$X_test, 
...
#' 
csbart <- function(X, Y, X_test, hypers = NULL, opts = Opts(), verbose = TRUE) {
...
TreeSelect <- function(X,Y, X_test, hypers = NULL, tree_start = 25, opts = Opts()) {
  ...
  fit <- csbart(X,Y,X_test,hypers, opts)
