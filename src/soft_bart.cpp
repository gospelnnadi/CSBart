...
// [[Rcpp::export]]
List CSBart(const arma::mat& X, const arma::vec& Y, const arma::mat& X_test,
...
// Add this block anywhere among the other Forest:: method implementations
// (e.g. directly above `arma::vec Forest::do_predict(const arma::mat& X)`).
// lower/upper are included even though do_predict()/GetW() never read them
// (only var/val/tau/mu matter for prediction) -- only needed if a tree
// loaded via set_trees() is ever grown further via GetLimits()-dependent
// proposals; cheap to include regardless.

Rcpp::List node_to_list(Node* n) {
  if (n->is_leaf) {
    return Rcpp::List::create(
      Rcpp::Named("is_leaf") = true,
      Rcpp::Named("mu")      = n->mu,
      Rcpp::Named("tau")     = n->tau
    );
  }
  return Rcpp::List::create(
    Rcpp::Named("is_leaf") = false,
    Rcpp::Named("var")     = n->var,
    Rcpp::Named("val")     = n->val,     // the real cutpoint field name
    Rcpp::Named("lower")   = n->lower,
    Rcpp::Named("upper")   = n->upper,
    Rcpp::Named("tau")     = n->tau,
    Rcpp::Named("left")    = node_to_list(n->left),
    Rcpp::Named("right")   = node_to_list(n->right)
  );
}

Node* list_to_node(Rcpp::List l, Node* parent) {
  Node* n = new Node();               // default ctor: is_leaf=true,
  // is_root=true, left=right=
  // parent=NULL, var=0, val=0,
  // lower=0, upper=1, tau=1, mu=0
  n->parent  = parent;
  ...
  if (n->is_leaf) {
    ...
    n->left  = n;    // matches Node()'s own convention: a leaf's
    n->right = n;    // left/right point to itself, never dereferenced
    // because every traversal checks is_leaf first
  } else {
    n->var   = Rcpp::as<int>(l["var"]);
    n->val   = Rcpp::as<double>(l["val"]);
    n->lower = Rcpp::as<double>(l["lower"]);
    n->upper = Rcpp::as<double>(l["upper"]);
    n->left  = list_to_node(Rcpp::as<Rcpp::List>(l["left"]),  n);
    n->right = list_to_node(Rcpp::as<Rcpp::List>(l["right"]), n);
  }
  return n;
}

Rcpp::List Forest::get_trees(Rcpp::IntegerVector tree_idx) {
  Rcpp::List out(tree_idx.size());
  for (int t = 0; t < tree_idx.size(); t++) {
    out[t] = node_to_list(trees[tree_idx[t]]);
  }
  return out;
}

void Forest::set_trees(Rcpp::List tree_lists, Rcpp::IntegerVector tree_idx) {
  if (tree_lists.size() != tree_idx.size())
    Rcpp::stop("set_trees: tree_lists and tree_idx must be the same length");
  for (int t = 0; t < tree_idx.size(); t++) {
    delete trees[tree_idx[t]];   // Node::~Node() already recursively
    // deletes children when !is_leaf --
    // confirmed in soft_bart.cpp, no
    // separate cleanup routine needed
    trees[tree_idx[t]] = list_to_node(Rcpp::as<Rcpp::List>(tree_lists[t]), NULL);
  }
}
...
RCPP_MODULE(mod_forest) {
  ...
  class_<Forest>("Forest")
    ...
    .method("get_trees", &Forest::get_trees)
    .method("set_trees", &Forest::set_trees);
}
